...
class _SalesreturnscreenState extends State<Salesreturnscreen> {
  ...
  Future<void> fetchTask() async {
    ...
    try {
      final cachedItems = await SalesReturnStorage.getSalesReturns();
    ...
    try {
      final freshItems = await SalesReturnService().fetchData();
      if (freshItems.isNotEmpty) {
        setState(() {
          tasKitem = freshItems;
        });
        // 3. Save to cache for subsequent opens
        await SalesReturnStorage.saveSalesReturns(freshItems);
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: CustomRefreshIndicator(
        onRefresh: () async {
          await SalesReturnStorage.clearCache();
          await fetchTask();
        },
        builder:
            (
